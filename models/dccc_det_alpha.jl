function build_dccc_det(generators, buses, lines, farms, α_det::Array{T, 1} where T <: Real, p_det::Array{T, 1} where T <: Real; output_level = 0)
    ...
    @constraint(m, d_lin == sum(p_det[i] * generators[i].pi2 for i in 1:n_generators))
    @constraint(m, vec(vcat(0.5, d_quad, C_rt * p_det)) in RotatedSecondOrderCone())
    @constraint(m, d_bil_c, sum(2 * generators[g].pi1 * α[g] * p_det[g] for g in 1:n_generators) == d_bil)
