function build_dccc_n2n_ab(generators, buses, lines, farms)
    ...
    @constraint(m, reserve1[u in 1:n_farms], sum(αp[i, u] for i in 1:n_generators) == 1)
    @constraint(m, reserve2[u in 1:n_farms], sum(αm[i, u] for i in 1:n_generators) == 1)
    ...
    @constraint(m, vec(vcat(r_uncert, 0.5, C_rt * pp_uncert, C_rt * pm_uncert)) in RotatedSecondOrderCone())
