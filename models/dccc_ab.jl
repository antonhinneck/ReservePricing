function build_dccc_ab(generators, buses, lines, farms)
    ...
    @constraint(m, cc1[i in 1:n_generators], p[i] + sum(μm) * (αm[i] - αp[i]) + za * sm * αm[i] <= generators[i].Pgmax)
    @constraint(m, cc2[i in 1:n_generators], -p[i] + sum(μp) * (αp[i] - αm[i]) + za * sp * αp[i] <= -generators[i].Pgmin)
    ...
    @constraint(m, approx2[i in 1:n_generators, j in 1:length(my_aprxs[i].coefs)], ucp[i] >= 0.5 * sum(μm) * my_aprxs[i].coefs[j][1] * (αm[i] - αp[i]))
