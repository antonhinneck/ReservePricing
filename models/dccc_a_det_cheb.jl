function build_dccc_a_det_cheb(generators, buses, lines, farms, pdet; output_level = 0)
        ...
        @expression(m, p_by_bus[i=1:n_buses], length(buses[i].genids) > 0 ? sum(pdet[k] for k in buses[i].genids) : 0.0)
        ...
        @constraint(m, d_lin == sum(pdet[i] * generators[i].pi2 for i in 1:n_generators))
        @constraint(m, vec(vcat(0.5, d_quad, C_rt * pdet)) in RotatedSecondOrderCone())
