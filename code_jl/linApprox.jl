...
function approx(generators::Vector{Generator}, g::Int64; error = 10e-9, segments_pu = 16, segments_total = nothing)

    left = generators[g].Pgmin
    right = generators[g].Pgmax
    if segments_total != nothing
        segments = segments_total
    else
        segments = ceil(right) * segments_pu
    end
...
for j in 1:n_generators
    push!(my_aprxs, aprx(approx(generators, j, segments_total = 1)...))
end
...
if true
    ...
    fig = figure(figsize=(3.5, 2))
    rc("font", family = "serif", style = "italic", size = 14)
    rc("text", usetex = true)
    rc("lines", linewidth = 1)

    ax = fig.add_axes([0.228,0.22,0.76,0.76])
    ...
    for i in 1:length(coefs)
        if true
            ...
            if i == 1
                plot([x1, x2],[y1, y2], color = grad_rgba[i], linewidth = 0.2, label = "Approx.")
            else
    ...
    plot(quad_x, quad_y, color = "black", linestyle = "solid", linewidth = 0.1, label = "\$c(p_{5})\$")
