function build_dccc_ab(generators, buses, lines, farms)
    ...
    @constraint(m, balance1, sum(αp[i] for i in 1:n_generators) == 1)
    @constraint(m, balance2, sum(αm[i] for i in 1:n_generators) == 1)
    ...
    #@constraint(m, vec(vcat(r_uncert, 0.5, C_rt' * αm * s)) in RotatedSecondOrderCone())
