function build_dccc_det_cheb(generators, buses, lines, farms, p_det::Array{T, 1} where T <: Real; output_level = 0)
        ...
        ##----------
        @variable(m, unc_c >= 0)
        @constraint(m, vec(vcat(0.5, unc_c, C_rt * α .* sum(σ_cheb))) in RotatedSecondOrderCone())
