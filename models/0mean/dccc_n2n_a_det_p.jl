function build_dccc_n2n_a_det_p(generators, buses, lines, farms, p_det; output_level = 0)
    ...
    @constraint(m, cc1[i in 1:n_generators], p[i] + za * pm_uncert[i] <= generators[i].Pgmax)
    @constraint(m, cc2[i in 1:n_generators], -p[i] + za * pp_uncert[i] <= -generators[i].Pgmin)
    ...
    @constraint(m, vec(vcat(0.5, u_quadm, C_rt * pm_uncert)) in RotatedSecondOrderCone())
    @constraint(m, vec(vcat(0.5, u_quadp, C_rt * pp_uncert)) in RotatedSecondOrderCone())

    ## Objective
    ##----------
    @objective(m, Min, u_quad + det_c)
