function build_dccc_ab(generators, buses, lines, farms)
    ...
    @constraint(m, cc1[i in 1:n_generators], p[i] + sum(μm) * αm[i] + za * sm * αm[i] <= generators[i].Pgmax)
    @constraint(m, cc2[i in 1:n_generators], -p[i] + sum(μp) * αp[i] + za * sp * αp[i] <= -generators[i].Pgmin)
